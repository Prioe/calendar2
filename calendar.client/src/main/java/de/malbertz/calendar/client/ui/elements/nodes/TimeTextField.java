...
public class TimeTextField extends TextField {
	...
	public void appendText(String text) {
		// Ignore this. Our text is always 8 characters long, we cannot append
		// anything
	}
	...
	public boolean deleteNextChar() {
		...
		if (selection.getLength() > 0) {
			...
			// If the caret preceeds a digit, replace that digit with a zero and
			// move the caret forward. Else just move the caret forward.
			int caret = this.getCaretPosition();
			if (caret % 3 != 2) { // not preceeding a colon
				String currentText = this.getText();
				setText(currentText.substring(0, caret) + "0"
						+ currentText.substring(caret + 1));
				success = true;
			}
	...
	public boolean deletePreviousChar() {
		...
		if (selection.getLength() > 0) {
			...
			// If the caret is after a digit, replace that digit with a zero and
			// move the caret backward. Else just move the caret back.
			int caret = this.getCaretPosition();
			if (caret % 3 != 0) { // not following a colon
				String currentText = this.getText();
				setText(currentText.substring(0, caret - 1) + "0"
	...
	public void replaceSelection(String replacement) {
		...
		if (selection.getLength() == 0) {
			...
			this.replaceText(selection.getStart(), selection.getEnd(),
					replacement);
		}
	}
	...
	public void replaceText(int begin, int end, String text) {
		if (begin == end) {
			...
			// characters being replaced, and if the replacement results in a
			// valid string:
