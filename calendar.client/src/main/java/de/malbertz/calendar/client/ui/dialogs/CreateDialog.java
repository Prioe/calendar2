...
public class CreateDialog extends Stage implements Initializable {
   ...
   public CalendarEntry waitForEntry() {
      showAndWait();
      if (!canceled) {
         LocalDate ld = datePicker.getValue() != null ? datePicker.getValue()
               : LocalDate.now();
         LocalTime start = startTimeTextField.getValue();
         LocalTime end = endTimeTextField.getValue();
         String name = nameTextField.getText();
         String desc = descTextArea.getText();
         CalendarEntry newEntry = new CalendarEntry(ld, start, end, name, desc);
         switch (mode) {
         case EDIT:
            Context
                  .getInstance()
         ...
         case NEW:
            Context.getInstance().getClient()
                  .sendCommand(new ServerCommand(Command.ADD, newEntry, null));
            break;
         default:
            break;
         }
   ...
   public void initialize(URL location, ResourceBundle resources) {
      ...
      startTimeTextField
            .setTooltip(new Tooltip(resources.getString("startTT")));
